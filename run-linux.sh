...
: "${EIC_SHELL_CVMFS_PREFIX:=/cvmfs/singularity.opensciencegrid.org}"

echo "::group::Checking if there is a working CVMFS mount"

if [ ! -d "${EIC_SHELL_CVMFS_PREFIX}" ]; then
  echo "The directory ${EIC_SHELL_CVMFS_PREFIX} cannot be accessed!"
...
if [ -z "${SANDBOX_PATH}" ]; then
  SANDBOX_PATH="${EIC_SHELL_CVMFS_PREFIX}/${EIC_SHELL_ORGANIZATION}/${EIC_SHELL_PLATFORM_RELEASE}"
  if [[ "${EIC_SHELL_RELEASE}" == *"dev"* ]]; then
    SANDBOX_PATH="${EIC_SHELL_CVMFS_PREFIX}/${EIC_SHELL_ORGANIZATION}/${EIC_SHELL_PLATFORM}:${EIC_SHELL_RELEASE}"
  fi
fi
